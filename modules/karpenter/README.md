...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_irsa"></a> [irsa](#module\_irsa) | terraform-aws-modules/iam/aws//modules/iam-role-for-service-accounts-eks | 5.60.0 |
...
| <a name="input_feature_gates"></a> [feature\_gates](#input\_feature\_gates) | Karpenter controller feature gates. Omitted keys use Karpenter chart defaults. | <pre>object({<br/>    node_repair                = optional(bool)<br/>    node_overlay               = optional(bool)<br/>    reserved_capacity          = optional(bool)<br/>    spot_to_spot_consolidation = optional(bool)<br/>    static_capacity            = optional(bool)<br/>  })</pre> | `{}` | no |
...
| <a name="input_resources"></a> [resources](#input\_resources) | container resources for Karpenter controller | <pre>object({<br/>    limits = optional(object({<br/>      cpu    = optional(string)<br/>      memory = optional(string)<br/>    }), {})<br/>    requests = optional(object({<br/>      cpu    = optional(string)<br/>      memory = optional(string)<br/>    }), {})<br/>  })</pre> | <pre>{<br/>  "limits": {<br/>    "cpu": "1",<br/>    "memory": "300Mi"<br/>  },<br/>  "requests": {<br/>    "cpu": "100m",<br/>    "memory": "256Mi"<br/>  }<br/>}</pre> | no |
| <a name="input_spot_to_spot_consolidation"></a> [spot\_to\_spot\_consolidation](#input\_spot\_to\_spot\_consolidation) | Deprecated: prefer feature\_gates.spot\_to\_spot\_consolidation. Used when feature\_gates.spot\_to\_spot\_consolidation is not set. | `bool` | `false` | no |
